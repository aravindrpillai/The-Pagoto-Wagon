...
<?php include_once("common/side_navigation.php") ?>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>Cups <small>Shop Admin Privilage</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="<?php echo base_url("/Home") ?>"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><a><i class="fa fa-dashboard"></i> Cups</a></li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">
			  <form action="<?php echo base_url("Cups/Index") ?>" method="POST">
				  <table>
					  <tr>
						  <td>
							  <select name="shop_id" class="form-control">
								  <?php foreach($shops as $shop): ?>
									<option value="<?php echo $shop["id"]?>" <?php echo $this->session->flashdata('cups_selected_shop_id_'.$shop["id"]) ?> > <?php echo $shop["name"]." - ".$shop["place"]?> </option>
								  <?php endforeach; ?>
							  </select>
						  </td>
						  <td>
              ...
              <div class="box-tools">
                <div class="input-group input-group-sm" style="width: 150px;">
                  <div class="input-group-btn">
                    <button onClick="addNewCup()" id="add_shop_btn" type="submit" class="btn btn-flat btn-success"><i class="fa fa-plus"></i> Add New Cup</button>
                  </div>
                </div>
              </div>
            </div>
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
				...
				<tr id="new_shop_tr">
                  <form action="<?php echo base_url('Cups/addCup') ?>" method="POST" enctype="multipart/form-data">
                  ...
                  <td>
					...
					<button type="reset" class="btn btn-flat btn-warning" onClick="addNewCup()"><i class="fa fa-close"></i></button>
				  </td>
				  <input type="hidden" name="shop_id" value="<?php echo $this->session->flashdata('cups_shop_id') ?>" >
				  </form>
                </tr>
				<?php foreach($cups as $key=>$data ): ?>
					<tr>
					  <form action="<?php echo base_url('Cups/Update') ?>" method="POST" enctype="multipart/form-data">
					  <td>
						<img id="<?php echo 'old_image_'.$data["id"] ?>" src="<?php echo base_url("assets/cups/".$data['image']) ?>" width="40px" height="40px">
						<input id="<?php echo 'new_image_'.$data["id"] ?>" type="file" name="image" class="image_form form-control">
					  </td>
					  <td><input readonly type="text" name="name" id="<?php echo 'name_'.$data['id'] ?>" value="<?php echo $data['name'] ?>" class="cups_form form-control"></td>
					  <td><input readonly type="text" name="price" id="<?php echo 'price_'.$data['id'] ?>" value="<?php echo $data['price'] ?>" class="cups_form form-control"></td>
					  <td><input readonly type="text" name="description" id="<?php echo 'description_'.$data['id'] ?>" value="<?php echo $data['description'] ?>" class="cups_form form-control"></td>
					  <td>
						<button type="button" onClick="editCupData('<?php echo $data["id"] ?>')" class="update_enable_btn btn btn-flat btn-info"><i class="fa fa-pencil"></i></button>
					  ...
					  <input type="hidden" name="cup_id" value="<?php echo $data["id"] ?>" >
...
<script>
	...
	if("<?php echo $this->session->flashdata('cups_display_form') ?>" == "1"){
		addNewCup();
	}
	...
	if("<?php echo $this->session->flashdata('cup_id') ?>" != ""){
		editCupData(<?php echo $this->session->flashdata('cup_id') ?>);
	}
	
	function editCupData(id){
		...
		$("#name_"+id).removeClass("cups_form");
		$("#name_"+id).removeAttr("readonly",false);
		$("#price_"+id).removeClass("cups_form");
		$("#price_"+id).removeAttr("readonly",false);
		$("#description_"+id).removeClass("cups_form");
