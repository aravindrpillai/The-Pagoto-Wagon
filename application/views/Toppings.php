...
<?php include_once("common/side_navigation.php") ?>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>Toppings <small>Shop Admin Privilage</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="<?php echo base_url("/Home") ?>"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><a><i class="fa fa-dashboard"></i> Toppings</a></li>
      </ol>
    </section>
              ...
              <div class="box-tools">
                <div class="input-group input-group-sm" style="width: 150px;">
                  <div class="input-group-btn">
                    <button onClick="addNewTopping()" id="add_shop_btn" type="submit" class="btn btn-flat btn-success"><i class="fa fa-plus"></i> Add New Topping</button>
                  </div>
                </div>
              </div>
            </div>
              ...
              <table class="table table-hover">
				...
				<?php foreach($toppings as $key=>$data ): ?>
					<tr>
					  ...
					  <td><input readonly type="text" name="name" id="<?php echo 'name_'.$data['id'] ?>" value="<?php echo $data['name'] ?>" class="toppings_form form-control"></td>
					  <td><input readonly type="text" name="price" id="<?php echo 'price_'.$data['id'] ?>" value="<?php echo $data['price'] ?>" class="toppings_form form-control"></td>
...
<script>
	$("#new_shop_tr").hide();
	var addNewToppingFlag = false;
	function addNewTopping(){
		if(addNewToppingFlag){
			$("#new_shop_tr").hide();
			$("#add_shop_btn").show();
			addNewToppingFlag = false;
		}else{
			$("#new_shop_tr").show();
			$("#add_shop_btn").hide();
			addNewToppingFlag = true;
		}
	}
	...
	if("<?php echo $this->session->flashdata('topping_id') ?>" != ""){
		editToppingData(<?php echo $this->session->flashdata('topping_id') ?>);
	}
	
	function editToppingData(id){
		...
		$("#name_"+id).removeClass("toppings_form");
		$("#name_"+id).removeAttr("readonly",false);
		$("#price_"+id).removeClass("toppings_form");
		$("#price_"+id).removeAttr("readonly",false);
		$("#description_"+id).removeClass("toppings_form");
